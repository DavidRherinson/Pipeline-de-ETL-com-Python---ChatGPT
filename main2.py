...
#para cada id na lista user_ids ele vai chamar a função usando os ids da lista para preocurar na api

# se get_user(id) for diferente de none  ele vai atribuir o retorno da funçaõ a miha list user
# " := " simbolo de igualar
...
def generate_ai_news(user):
    completion = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system", 
                "content": "Você é um investidor espicialista em criptomoedas"
            },
            {
                "role": "user", 
                "content": f"Crie uma mensagem para {user['name']} sobre a importância dos investimentos em biticoins (máximo de 100 caracteres, a mensagem devera ser persoasiva e possui ao menos 3 linhas)"
            }
        ]
    )
